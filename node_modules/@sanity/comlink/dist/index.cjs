...
const listenInputFromContext = (config) => ({
...
}), createChannelMachine = () => xstate.setup({
  types: {},
  actors: {
  ...
  actions: {
    ...
    "set connection config": xstate.assign({
      connectionId: ({ event }) => (xstate.assertEvent(event, "message.received"), event.message.data.connectionId),
      target: ({ event }) => (xstate.assertEvent(event, "message.received"), event.message.source || void 0),
      targetOrigin: ({ event }) => (xstate.assertEvent(event, "message.received"), event.message.origin)
    })
  },
  guards: {
...
exports.createChannelMachine = createChannelMachine;

  ...
  return function(editor) {
    ...
    return editor.normalizeNode = (nodeEntry) => {
      ...
      if (editor.isTextBlock(node)) {
        ...
        for (const [child, childPath] of children) {
          ...
          if (editor.isTextSpan(child) && editor.isTextSpan(nextNode) && child.marks?.every((mark) => nextNode.marks?.includes(mark)) && nextNode.marks?.every((mark) => child.marks?.includes(mark))) {
            debug$j("Merging spans", JSON.stringify(child, null, 2), JSON.stringify(nextNode, null, 2)), editorActor.send({
              type: "normalizing"
      ...
      if (editor.isTextBlock(node) && !Array.isArray(node.markDefs)) {
        debug$j("Adding .markDefs to block node"), editorActor.send({
          type: "normalizing"
      ...
      if (editor.isTextSpan(node) && !Array.isArray(node.marks)) {
        debug$j("Adding .marks to span node"), editorActor.send({
          type: "normalizing"
        }), Transforms.setNodes(editor, {
          marks: []
        }, {
      ...
      if (editor.isTextSpan(node)) {
        ...
        if (editor.isTextBlock(block) && node.text === "" && annotations && annotations.length > 0) {
          debug$j("Removing annotations from empty span node"), editorActor.send({
            type: "normalizing"
          }), Transforms.setNodes(editor, {
        ...
        for (const [child, childPath] of Node.children(editor, path))
          if (editor.isTextSpan(child)) {
            ...
            if (orphanedAnnotations.length > 0) {
              debug$j("Removing orphaned annotations from span node"), editorActor.send({
                type: "normalizing"
              }), Transforms.setNodes(editor, {
      ...
      if (editor.isTextBlock(node)) {
        ...
        if (markDefs.length !== newMarkDefs.length) {
          debug$j("Removing duplicate markDefs"), editorActor.send({
            type: "normalizing"
          }), Transforms.setNodes(editor, {
      ...
      if (editor.isTextBlock(node) && !editor.operations.some((op) => op.type === "merge_node" && "markDefs" in op.properties && op.path.length === 1)) {
        ...
        if (node.markDefs && !isEqual(newMarkDefs, node.markDefs)) {
          debug$j("Removing markDef not in use"), editorActor.send({
            type: "normalizing"
          }), Transforms.setNodes(editor, {
            markDefs: newMarkDefs
          }, {
      ...
      if (op.type === "merge_node" && op.path.length === 1 && "markDefs" in op.properties && op.properties._type === types.block.name && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0 && op.path[0] - 1 >= 0) {
        ...
        if (editor.isTextBlock(targetBlock)) {
          ...
          debug$j("Copying markDefs over to merged block", op), Transforms.setNodes(editor, {
            markDefs: newMarkDefs
          }, {
            at: targetPath,
            voids: !1
...
}, debug$i = debugWithName("API:editable");
  ...
  return function(editor) {
    return portableTextEditor.setEditable({
      ...
      insertChild: (type, value) => {
        ...
        return isSpanNode && focusNode._type !== types.span.name && (debug$i("Inserting span child next to inline object child, moving selection + 1"), editor.move({
          distance: 1,
          unit: "character"
      ...
      delete: (selection, options) => {
        if (selection) {
          ...
          if (range) {
            ...
            options?.mode === "blocks" && (debug$i("Deleting blocks touched by selection"), Transforms.removeNodes(editor, {
              at: range,
              voids: !0,
              match: (node) => editor.isTextBlock(node) || !editor.isTextBlock(node) && Element$1.isElement(node)
            })), options?.mode === "children" && (debug$i("Deleting children touched by selection"), Transforms.removeNodes(editor, {
              at: range,
              voids: !0,
      ...
      removeAnnotation: (type) => {
        debug$i("Removing annotation", type), Editor.withoutNormalizing(editor, () => {
          if (editor.selection)
...
const debug$h = debugWithName("applyPatches"), debugVerbose$4 = debug$h.enabled && !0;
function createApplyPatch(schemaTypes) {
  return (editor, patch) => {
    let changed = !1;
    debugVerbose$4 && (debug$h(`
...
function insertPatch(editor, patch, schemaTypes) {
  ...
  if (patch.path.length === 1) {
    ...
    return debug$h(`Inserting blocks at path [${normalizedIdx2}]`), debugState(editor, "before"), Transforms.insertNodes(editor, blocksToInsert, {
      at: [normalizedIdx2]
  ...
  return debug$h(`Inserting children at path ${childInsertPath}`), debugState(editor, "before"), childrenToInsert && Element$1.isElement(childrenToInsert[0]) && Transforms.insertNodes(editor, childrenToInsert[0].children, {
    at: childInsertPath
  }), debugState(editor, "after"), !0;
}
function setPatch(editor, patch) {
  ...
  if (debugState(editor, "before"), isTextBlock && child && childPath) {
    ...
    }), debug$h("Setting children"), block.children.forEach((c2, cIndex) => {
      editor.apply({
        type: "remove_node",
...
function unsetPatch(editor, patch) {
  ...
  if (patch.path.length === 1) {
    ...
    return debug$h(`Removing block at path [${blockIndex}]`), debugState(editor, "before"), Transforms.removeNodes(editor, {
      at: [blockIndex]
    }), debugState(editor, "after"), !0;
  }
  return editor.isTextBlock(block) && patch.path[1] === "children" && patch.path.length === 3 ? !child || !childPath ? (debug$h("Child not found"), !1) : (debug$h(`Unsetting child at path ${JSON.stringify(childPath)}`), debugState(editor, "before"), debugVerbose$4 && debug$h(`Removing child at path ${JSON.stringify(childPath)}`), Transforms.removeNodes(editor, {
    at: childPath
...
function debugState(editor, stateName) {
  debugVerbose$4 && (debug$h(`Children ${stateName}:`, JSON.stringify(editor.children, null, 2)), debug$h(`Selection ${stateName}: `, JSON.stringify(editor.selection, null, 2)));
}
...
const debug$g = debugWithName("plugin:withUndoRedo"), debugVerbose$3 = debug$g.enabled && !1, SAVING = /* @__PURE__ */ new WeakMap(), REMOTE_PATCHES = /* @__PURE__ */ new WeakMap(), UNDO_STEP_LIMIT = 1e3, isSaving = (editor) => {
...
function transformOperation(editor, patch, operation, snapshot, previousSnapshot) {
  debugVerbose$3 && (debug$g(`Adjusting '${operation.type}' operation paths for '${patch.type}' patch`), debug$g(`Operation ${JSON.stringify(operation)}`), debug$g(`Patch ${JSON.stringify(patch)}`));
  ...
  if (patch.type === "unset" && patch.path.length === 1) {
    ...
    return "path" in transformedOperation && Array.isArray(transformedOperation.path) && transformedOperation.path[0] === unsetBlockIndex ? (debug$g("Skipping transformation that targeted removed block"), []) : (debugVerbose$3 && (debug$g(`Selection ${JSON.stringify(editor.selection)}`), debug$g(`Adjusting block path (-1) for '${transformedOperation.type}' operation and patch '${patch.type}'`)), [adjustBlockPath(transformedOperation, -1, unsetBlockIndex)]);
  }
  if (patch.type === "unset" && patch.path.length === 0)
          ...
          for (const hotkey in activeHotkeys[cat]) {
            ...
            if (isHotkey(hotkey, event.nativeEvent)) {
              ...
              if (possibleMark) {
                ...
                debug$8(`HotKey ${hotkey} to toggle ${mark}`), editorActor.send({
                  type: "behavior event",
                  behaviorEvent: {
                    type: "decorator.toggle",
                    decorator: mark

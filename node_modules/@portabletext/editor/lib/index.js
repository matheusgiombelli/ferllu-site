    ...
    return editor.normalizeNode = (nodeEntry) => {
      ...
      if (op.type === "merge_node" && op.path.length === 1 && "markDefs" in op.properties && op.properties._type === types2.block.name && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0 && op.path[0] - 1 >= 0) {
        ...
        if (editor.isTextBlock(targetBlock)) {
          ...
          debug$j("Copying markDefs over to merged block", op), slate.Transforms.setNodes(editor, {
            markDefs: newMarkDefs
          }, {
            at: targetPath,
            voids: !1
  ...
  return function(editor) {
    return portableTextEditor.setEditable({
      ...
      insertChild: (type, value) => {
        ...
        return isSpanNode && focusNode._type !== types$1.span.name && (debug$i("Inserting span child next to inline object child, moving selection + 1"), editor.move({
          distance: 1,
          unit: "character"
      ...
      delete: (selection, options) => {
        if (selection) {
          ...
          if (range) {
            ...
            options?.mode === "blocks" && (debug$i("Deleting blocks touched by selection"), slate.Transforms.removeNodes(editor, {
              at: range,
              voids: !0,
              match: (node) => editor.isTextBlock(node) || !editor.isTextBlock(node) && slate.Element.isElement(node)
            })), options?.mode === "children" && (debug$i("Deleting children touched by selection"), slate.Transforms.removeNodes(editor, {
              at: range,
              voids: !0,
      ...
      removeAnnotation: (type) => {
        debug$i("Removing annotation", type), slate.Editor.withoutNormalizing(editor, () => {
          if (editor.selection)
...
function insertPatch(editor, patch, schemaTypes) {
  ...
  if (patch.path.length === 1) {
    ...
    return debug$h(`Inserting blocks at path [${normalizedIdx2}]`), debugState(editor, "before"), slate.Transforms.insertNodes(editor, blocksToInsert, {
      at: [normalizedIdx2]
  ...
  return debug$h(`Inserting children at path ${childInsertPath}`), debugState(editor, "before"), childrenToInsert && slate.Element.isElement(childrenToInsert[0]) && slate.Transforms.insertNodes(editor, childrenToInsert[0].children, {
    at: childInsertPath
  }), debugState(editor, "after"), !0;
}
...
function unsetPatch(editor, patch) {
  ...
  if (patch.path.length === 1) {
    ...
    return debug$h(`Removing block at path [${blockIndex}]`), debugState(editor, "before"), slate.Transforms.removeNodes(editor, {
      at: [blockIndex]
    }), debugState(editor, "after"), !0;
  }
  return editor.isTextBlock(block) && patch.path[1] === "children" && patch.path.length === 3 ? !child || !childPath ? (debug$h("Child not found"), !1) : (debug$h(`Unsetting child at path ${JSON.stringify(childPath)}`), debugState(editor, "before"), debugVerbose$4 && debug$h(`Removing child at path ${JSON.stringify(childPath)}`), slate.Transforms.removeNodes(editor, {
    at: childPath
